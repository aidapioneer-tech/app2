...
| Высота встройки | `index.vue → fitFrame()` меряет реальный контент-элемент (`contentEl` — div с паддингами внутри `B24DashboardPanel#body`) и ставит высоту фрейма явно: `$b24.parent.resizeWindow(width, ceil(scrollHeight × HEIGHT_BUFFER))`, где `HEIGHT_BUFFER = 1.1` (+10% запас — иначе из-за хрома обёртки остаётся скролл). **Почему не `fitWindow()`:** контент внутри dashboard-обёртки (`B24DashboardGroup`/`Panel`) во весь вьюпорт с внутренним скроллом, поэтому `fitWindow()` (меряет высоту документа) видит вьюпорт, а не контент. `fitWindow()` — только фолбэк. Вызов после `nextTick` + кадра, на initial/refresh и через `ResizeObserver` (debounce) при динамике/ресайзе (resizeWindow ставит и ширину — наблюдатель возвращает адаптивность). |
