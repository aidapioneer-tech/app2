...
| Шапка (Header) | Presentation-only: получает `title/subtitle/metrics[]`. Метрики формирует `headerMetrics.ts → buildHeaderMetrics(totals, currency, mode)` (единая точка правды). Клиент: Сумма сделки (с НДС) / Маржинальность / **Прибыль (без НДС)**. Подрядчик: Сумма расхода / Маржинальность / **Прибыль (без НДС)**. Всё `totals.plan`. Третья метрика — `profit` (`incomeNet − expenseTotal`), **не** `incomeNet`: доход без вычета подрядчиков вводит в заблуждение ([aida#93](https://github.com/aidapioneer-tech/aida/issues/93)) и дублирует «Сумму сделки» при НДС = 0; прибыль согласуется с маржой (`profit / incomeNet`). Покрыто `headerMetrics.test.ts`. Без разделения план/факт, без прогресс-бара. |
